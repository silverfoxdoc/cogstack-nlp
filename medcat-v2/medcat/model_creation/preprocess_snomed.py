...
class FileFormatDescriptor:
    ...
    relationship: str
    refset: str
    # for concept, description, and relationship (but not refset)
...
class SupportedBundles(Enum):
    UK_CLIN = BundleDescriptor(
        extensions=[
            SupportedExtension.INTERNATIONAL, SupportedExtension.UK_CLINICAL,
            SupportedExtension.UK_CLINICAL_REFSET,
            SupportedExtension.UK_EDITION],
...
class Snomed:
    ...
    This class is used to create a SNOMED CT concept DataFrame ready for
    MedCAT CDB creation.

    Attributes:
        data_path (str): Path to the unzipped SNOMED CT folder.
        release (str): Release of SNOMED CT folder.
        uk_ext (bool, optional): Specifies whether the version is a
            SNOMED UK extension released after 2021. Defaults to False.
        uk_drug_ext (bool, optional): Specifies whether the version
            is a SNOMED UK drug extension. Defaults to False.
        au_ext (bool, optional): Specifies whether the version is a
            AU release. Defaults to False.
    """
    ...
    def _determine_bundle(cls, data_path) -> Optional[SupportedBundles]:
        ...
        for bundle in SupportedBundles:
            ...
            exp_names = [(ext.value.exp_name_in_folder,
                          ext.value.exp_2nd_part_in_folder)
                         for ext in bundle.value.extensions]
            if match_partials_with_folders(exp_names, folder_names):
                return bundle
        return None

    def _set_extension(self, release: str, extension: SupportedExtension
                       ) -> None:
        ...
        if (extension in (SupportedExtension.UK_CLINICAL,
                          SupportedExtension.UK_DRUG) and
    ...
    def _determine_extension(cls, folder_path: str,
        ...
        for extension in SupportedExtension:
            ...
            if (extension.value.exp_2nd_part_in_folder and
                ...
                continue
            return extension
        ext_names_folders = ",".join(
            [f"{ext.name} ({ext.value.exp_name_in_folder})"
    ...
    def _determine_release(cls, folder_path: str, strict: bool = True,
        ...
        if match is None and strict:
            raise UnkownSnomedReleaseException(
                f"No version found in '{folder_path}'")
        elif match is None:
    ...
    def _check_path_and_release(self):
        """
        This function checks the path and release of the SNOMED CT data
        provided.

        It looks for the "Snapshot" folder within the data path, and if
        it's not found, it looks for any folder containing the name "SnomedCT".
        It then stores the path and release in separate lists.
        If no valid paths are found, it raises a FileNotFoundError.

        Returns:
            tuple: a tuple containing two lists, the first one is a list of
                the paths where the data is located and the second is a list
                of the releases of the data.

        Raises:
            FileNotFoundError: If the path to the SNOMED CT directory is
                incorrect.
        """
        snomed_releases = []
        paths = []
        ...
        if "Snapshot" in os.listdir(self.data_path):
            ...
            snomed_releases.append(
                self._determine_release(self.data_path, strict=True))
    ...
    def _refset_df2dict(self, refset_df: pd.DataFrame) -> dict:
        """
        This function takes a SNOMED refset DataFrame as an input and converts
        ...
        Returns:
            dict: mapping from SNOMED CT codes as key and the refset metadata
                list of dictionaries as values.
        """
        refset_dict = refset_df.groupby('referencedComponentId').apply(
            lambda group: [{'code': row['mapTarget'],
                            'mapGroup': row['mapPriority'],
                            'mapPriority': row['mapPriority'],
                            'mapRule': row['mapRule'],
                            'mapAdvice': row['mapAdvice']}
                           for _, row in group.iterrows()]).to_dict()
        return refset_dict

    def _map_snomed2refset(self):
        """
        Maps SNOMED CT concepts to refset mappings provided in the SNOMED CT
        release package.

        This function maps SNOMED CT concepts using the refset mappings in
        the Snapshot/Refset/Map directory.
        The refset mappings can either be ICD-10 codes in international
        releases or OPCS4 codes for SNOMED UK_extension, if available.

        Returns:
            pd.DataFrame: Dataframe containing SNOMED CT to refset mappings
                and metadata.
            OR
            tuple: Tuple of dataframes containing SNOMED CT to refset mappings
                and metadata (ICD-10, OPCS4), if uk_ext is True.
        """
        dfs2merge = []
        for i, snomed_release in enumerate(self.snomed_releases):
            self._set_extension(snomed_release, self.exts[i])
            refset_terminology = os.path.join(
                self.paths[i], PER_FILE_TYPE_PATHS[RefSetFileType.refset])
            ...
            for f in os.listdir(refset_terminology):
                ...
                if m:
                    snomed_v = m.group(1)
            mappings = parse_file(
                f'{refset_terminology}'
                f'/{icd10_ref_set}_{snomed_v}_{snomed_release}.txt')
            mappings = mappings[mappings.active == '1']
            icd_mappings = mappings.sort_values(
                by=['referencedComponentId', 'mapPriority',
                    'mapGroup']).reset_index(drop=True)
            dfs2merge.append(icd_mappings)
        mapping_df = pd.concat(dfs2merge)
        del dfs2merge
        if any(ext in (SupportedExtension.UK_CLINICAL,
                       SupportedExtension.UK_DRUG)
               for ext in self.exts):
